...
- 이미 열린 View에 Open 요청 시 `UI_ALREADY_OPEN` 반환
- 이미 닫힌 View에 Close 요청 시 `UI_ALREADY_CLOSED` 반환
- Opening 또는 Closing 중 중복 요청 시 `UI_BUSY` 반환
- CanvasGroup 누락 시 `UI_MISSING_CANVAS_GROUP` 반환
...
- Screen Navigation 중 `UI_BUSY` 반환
...
- 처리할 대상이 없을 경우 `UI_NO_BACK_TARGET` 반환
- Blocking Overlay 존재 시 `UI_BACK_BLOCKED` 반환
